...
/*************************** Enter your code here ****************************/
	...
	while(XUartPs_IsReceiveData(XPAR_XUARTPS_0_BASEADDR))
    {
        ...
        if (xQueueSendToBackFromISR(xRxQueue, &receive_buffer, &xHigherPriorityTaskWoken)){
        	byteCount++;
        }
    }
/*****************************************************************************/
...
/*************************** Enter your code here ****************************/
		...
		if (xQueueReceiveFromISR(xTxQueue, data, taskToSwitch)) {
            XUartPs_SendByte(XPAR_XUARTPS_0_BASEADDR, *data);
        }
/*****************************************************************************/
    }
}
...
/*************************** Enter your code here ****************************/
	...
	while (myReceiveData() == pdTRUE) {
		if (xQueueReceive(xRxQueue, &recv, 0) == pdPASS) {
            // Data was successfully received and stored in recv
        }
	}
	taskEXIT_CRITICAL();
	return recv;
